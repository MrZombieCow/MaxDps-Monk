...
function Monk:Windwalker()
	...
	if talents[WW.FistOfTheWhiteTiger] and cooldown[WW.FistOfTheWhiteTiger].ready and
		...
		(energyMax < 4 and cooldown[WW.FistsOfFury].remains < 1.5) or (cooldown[WW.WeaponsOfOrder].remains < 2 and (Monk.db.AnyAsCooldown or not Monk.db.WeaponsOfOrderAsCooldown))) then
...
function Monk:WindwalkerCooldown()
	...
	if cooldown[WW.WeaponsOfOrder].ready and cooldown[WW.RisingSunKick].remains < gcd * 2 and
	(Monk.db.AnyAsCooldown or not Monk.db.WeaponsOfOrderAsCooldown)
	then
